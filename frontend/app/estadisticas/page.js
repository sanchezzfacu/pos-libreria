...
export default function EstadisticasPage() {
  ...
  // El valor del inventario es una foto de "ahora mismo" — no depende del
  // selector de rango de fechas de arriba, así que se carga aparte, una
  // sola vez.
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        {inventario && (
          <div className="card border-ink-700/15 bg-ink-900">
            <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
              <h2 className="font-semibold text-white">Valor del inventario</h2>
              <span className="text-xs text-white/50">
                Ahora mismo · no depende del rango de fechas de abajo
              </span>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <CardOscura
                label="A precio de costo"
                valor={inventario.valorCosto}
                info="Cuánto pagaste en total por todo lo que tenés en stock ahora mismo."
              />
              <CardOscura
                label="A precio de venta"
                valor={inventario.valorVenta}
                info="Cuánto recaudarías si vendieras absolutamente todo el stock actual a precio de lista."
              />
              <CardOscura
                label="Ganancia potencial"
                valor={inventario.gananciaPotencial}
                destacado
                info="La diferencia entre vender todo el stock y lo que te costó — la ganancia bruta si se vendiera todo."
              />
            </div>
            <p className="text-xs text-white/40 mt-3">
              {inventario.productosConStock} productos con stock cargado ·{" "}
              {inventario.unidades} unidades en total
            </p>
          </div>
        )}
          ...
          <>
            ...
            <div className="card">
              <h2 className="font-semibold text-ink-900 mb-4">
                Ventas y ganancia neta por día
              </h2>
                ...
                <div style={{ width: "100%", height: 280 }}>
                  <ResponsiveContainer>
                    <ComposedChart data={data.porDia}>
                      ...
                      <XAxis
                        dataKey="fecha"
                        tick={{ fontSize: 11, fill: "#3E6E7A" }}
                        tickLine={false}
                        axisLine={{ stroke: "#D7E2E4" }}
                      />
                      <YAxis
                        tick={{ fontSize: 11, fill: "#3E6E7A" }}
                        tickLine={false}
            ...
            <div className="card">
              ...
              <p className="text-sm text-ink-400 mb-4">
                Para saber qué está funcionando y reponer a tiempo antes de
                quedarte sin stock.
              </p>
              {ranking.length === 0 ? (
                      ...
                      {ranking.slice(0, 20).map((r, i) => (
                        <tr key={r.productId || r.descripcion}>
                          <td className="text-ink-900">
                            <span className="text-ink-300 font-mono text-xs mr-2">
                              #{i + 1}
                            </span>
                            {r.descripcion}
                          </td>
                          <td className="price text-right font-semibold">
                            {r.cantidad}
                          </td>
                          <td className="price text-right text-ink-400">
                          ...
                          <td className="text-right">
                            {r.stockActual === null ? (
                              ...
                              <span
                                className={`price ${r.stockActual <= 0 ? "text-red-600 font-semibold" : "text-ink-600"}`}
                              >
                                {r.stockActual}
                              </span>
                            )}
...
function Card({
  ...
  return (
    <div
      className={`group relative card !p-4 ${destacado ? "border-ink-700/20 bg-ink-50/50" : ""}`}
    >
...
// Variante oscura del Card, para la sección de "Valor del inventario" —
// a propósito tiene otro fondo (ink-900) para que se note de un vistazo
// que es una sección distinta de los KPIs de ventas por rango de fechas.
function CardOscura({ label, valor, destacado = false, info }) {
  // Si viene como string formateado del backend (ej: "1.234.567,89"), remueve la coma y los centavos.
  // Si viene como número puro, lo formatea sin decimales.
  const valorMostrar =
    typeof valor === "string"
      ? valor.split(",")[0]
  ...
  return (
    <div className="group relative">
      <p
        className={`text-xl font-semibold price ${destacado ? "text-stamp-400" : "text-white"}`}
      >
        ${valorMostrar}
      </p>
      <p className="text-xs text-white/50 mt-0.5 flex items-center gap-1">
        {label}
        {info && (
          <span className="w-3.5 h-3.5 rounded-full bg-white/10 text-white/70 text-[10px] leading-[14px] text-center font-semibold cursor-help">
            i
          </span>
        )}
      </p>
      {info && (
        <div className="pointer-events-none absolute left-0 bottom-full mb-2 w-52 opacity-0 group-hover:opacity-100 transition-opacity z-20">
          <div className="bg-white text-ink-900 text-xs rounded-lg px-3 py-2 shadow-lg border border-ink-100">
            {info}
          </div>
        </div>
      )}
